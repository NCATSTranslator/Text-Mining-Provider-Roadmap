...
class TestCooccurrenceKGX(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:  # pragma: no cover
        ...
        with open('cooccurrence_json_attributes.yml', 'r') as infile:
            cls.json_attributes_schema = yaml.safe_load(infile)
        if not os.path.isfile("cooccurrence_edges.tsv.gz"):
            urlretrieve(cls.cooccurrence_edges_file_url, 'cooccurrence_edges.tsv.gz')
        with gzip.open('cooccurrence_edges.tsv.gz', 'rb') as edge_file:
            for i in range(cls.test_line_count):
                cls.test_edge_lines.append(edge_file.readline().decode('utf-8'))
        if not os.path.isfile("cooccurrence_nodes.tsv.gz"):
            urlretrieve(cls.cooccurrence_nodes_file_url, 'cooccurrence_nodes.tsv.gz')
        with gzip.open('cooccurrence_nodes.tsv.gz', 'rb') as node_file:
            for i in range(cls.test_line_count):
            ...
            for line in self.test_edge_lines:
                ...
                for property_key, property_value in attribute.items():
                    if type(property_value) == list:
                        for inner_attribute in property_value:
                            ...
                            for inner_key, inner_value in inner_attribute.items():
                                ...
                                if inner_value.startswith('<') and inner_value.endswith('>'):
                                    ...
                                    self.assertIsNotNone(regex.search(text))
                                else:
                        ...
                        self.assertIsNotNone(regex.search(str(json_object[property_key])))
                    else:
